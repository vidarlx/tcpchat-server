...
ChatServer.prototype.run = function() {
    ...
    var srv = net.createServer(function(conn) {
        ...
        function onConnectionClose() {
            delete that._users[nickname];
            that.broadcast("User " + nickname + " left chat \r\n> ", true);
        }
        ...
        that._commands.addListener('help', function(commands) {
            var cmds = [];
            cmds.push("---------- HELP ------------");
            cmds.push("\rAvailable commands:");
            for(command in commands) {
                var tmp = commands[command];
                cmds.push("\r/" + command + "\t\t" + tmp.dsc);
            }
            cmds = cmds.join("\n");
            conn.write(cmds + "\r\n");
        });
